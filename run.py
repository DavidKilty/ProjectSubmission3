    ...
    while True:
        try:
            ...
            if (row, col) in shots_taken:
                raise ValueError("You have already targeted these coordinates.")  # Avoid repeating input
            return row, col
...
def computer_turn(board, shots_taken):
    while True:
        row = random.randint(0, BOARD_SIZE - 1)  # Generate random row
        col = random.randint(0, BOARD_SIZE - 1)  # Generate random collumn
        if (row, col) not in shots_taken and board[row][col] in ('~', 'S'):  # Avoid computer repeating input
...
def play_game():  # Game play loop
    ...
    while True:
        ...
        if all_snakes_crushed(computer_board):  # For user winning
        ...
        if all_snakes_crushed(player_board):  # For user losing
